...
const Home: React.FC<HomeProps> = ({ onSelectApp, title, subtitle, apps }) => {
  ...
  const appListContainerClasses =
    displayedApps.length > 1
      ? 'grid w-full max-w-screen-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3'
        ...
        if (settings) {
            ...
            reader.onloadend = () => {
                if (typeof reader.result === 'string') {
                    openLayerComposer([reader.result]);
                }
            };
            reader.readAsDataURL(file);
        } else {
            toast.error("Không tìm thấy dữ liệu cài đặt trong file, hoặc file không phải là ảnh được hỗ trợ.");
        }
    } catch (e) {
