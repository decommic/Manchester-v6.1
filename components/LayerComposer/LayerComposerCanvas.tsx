...
export const LayerComposerCanvas: React.FC<LayerComposerCanvasProps> = ({
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            const isDuplicate = (e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'j';
            const isDeselect = (e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'd';
            const isMoveUp = (e.metaKey || e.ctrlKey) && e.key === ']';
            const isMoveDown = (e.metaKey || e.ctrlKey) && e.key === '[';
            ...
            if (selectedLayers.length > 0) {
                if (isDelete) {
                    ...
                    e.preventDefault();
                    onMultiLayerAction('group-up');
                } else if (isMoveDown) {
                    e.preventDefault();
                    onMultiLayerAction('group-down');
                }
            }

            // Simple key presses (no modifiers)
    ...
    }, [selectedLayers.length, deleteSelectedLayers, duplicateSelectedLayers, setSelectedLayerIds, onMultiLayerAction, setActiveCanvasTool]);
